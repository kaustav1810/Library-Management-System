...
export const BookCheckoutPage = () => {
  ...
  const [reviewLoading, isReviewLoading] = useState(true);
  ...
  const [isReviewedLoading, setIsReviewedLoading] = useState(false);
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      if (loadedReviews.length > 0) {
        const avgRating = (
          Math.round((weightedStarRating / loadedReviews.length) * 2) / 2
        ).toFixed(1);
  ...
  useEffect(() => {
    const fetchUserCurrentLoansCount = async () => {
      if (authState && authState.isAuthenticated) {
        const url =
          `${process.env.REACT_APP_API}/books/secure/checkout/currentLoans`;
  ...
  useEffect(() => {
    ...
    fetchUserBookCheckedout().catch((err: any) => {
      setIsLoadingCheckedout(false);
      setHttpError(err.message);
    });
  }, [authState, isCheckedout]);
  ...
  const postReview = async (starRating: number, reviewDescription: string) => {
    ...
    let newReview = new ReviewRequest(bookId, starRating, reviewDescription);
      ...
      <div className="container d-none d-lg-block">
        <div className="row mt-5">
          <div className="container col-sm-2 col-md-2">
            {book?.img ? (
              <img src={book?.img} alt="book" height="400" width="300" />
            ) : (
              <img
